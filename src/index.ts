...
app.post('/api/login', async (c) => {
  ...
  const user = db.query('SELECT * FROM usuarios WHERE username = ? AND password = ?').get(username, password);
...
app.post('/api/license/generate', async (c) => {
  ...
  const lic = await gerarChave(plan, maxActivations, durationValue);
...
app.post('/api/license/validate', async (c) => {
  ...
  const row = db.query('SELECT * FROM licencas WHERE license_key = ?').get(license_key);
  if (!row) return c.json({ ok: false, valid: false, mensagem: 'Chave não encontrada' }, 404);
  
  const now = Math.floor(Date.now() / 1000);
  if (now > row.expires_at) return c.json({ ok: false, valid: false, mensagem: 'Chave expirada' }, 403);
  if (row.status !== 'ativa') return c.json({ ok: false, valid: false, mensagem: 'Chave revogada' }, 403);
  
  return c.json({ ok: true, valid: true, plan: row.plan, expires_at: new Date(row.expires_at * 1000).toISOString(), max_activations: row.max_activations });
});
...
app.get('/api/license/list', async (c) => {
  ...
  const rows = db.query('SELECT license_key, plan, status, expires_at, comprador FROM licencas ORDER BY id DESC').all();
  return c.json(rows.map((r: any) => ({ license_key: r.license_key, plan: r.plan, status: r.status, expires_at: new Date(r.expires_at * 1000).toISOString(), comprador: r.comprador })));
});
